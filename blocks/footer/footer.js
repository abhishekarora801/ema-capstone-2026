...
// Inline SVG brand icons — the WKND source used an icon font that did not
// carry over, so the footer social links are rendered as inline SVG buttons.
const SOCIAL_ICONS = {
...
export default async function decorate(block) {
  ...
  // Tag the four fragment sections: brand, nav, social, legal.
  ...
  // Convert the social text links into a horizontal row of inline-SVG icons.
  const social = footer.querySelector('.footer-social');
  if (social) {
    const list = social.querySelector('ul');
    if (list) list.classList.add('footer-social-links');
    [...social.querySelectorAll('a')].forEach((a) => {
      const meta = iconFor(a);
      if (meta) {
